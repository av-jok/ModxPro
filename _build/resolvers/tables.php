...
if ($transport->xpdo) {
    ...
    switch ($options[xPDOTransport::PACKAGE_ACTION]) {
        ...
        case xPDOTransport::ACTION_UPGRADE:
            $modelPath = $modx->getOption('modxpro_core_path', null,
                    $modx->getOption('core_path') . 'components/modxpro/') . 'model/';
            $modx->addPackage('modxpro', $modelPath);

            $manager = $modx->getManager();
            $objects = [];
            $schemaFile = MODX_CORE_PATH . 'components/modxpro/model/schema/modxpro.mysql.schema.xml';
        ...
        // modUserProfile
        $modx->map['modUserProfile']['package'] = 'modx';
        $modx->map['modUserProfile']['version'] = '1.1';
        $modx->map['modUserProfile']['table'] = 'user_attributes';
        $modx->map['modUserProfile']['extends'] = 'xPDOSimpleObject';
        $c = $modx->prepare("SHOW COLUMNS IN {$modx->getTableName('modUserProfile')}");
        $c->execute();
        $found = false;
        while ($row = $c->fetch(PDO::FETCH_ASSOC)) {
